...
class ColorFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelno >= logging.ERROR:
            formatted = f"\033[0;31m{formatted}\033[0m"
        elif record.levelno >= logging.WARNING:
            formatted = f"\033[0;33m{formatted}\033[0m"
...
def _log_output(stream, logger, log_level=_log_level, observer=None):
...
class ManagedProcess:
    ...
    def start(self):
        ...
        _log_output(self._process.stdout, self._logger, self._log_level, self._observer)
        _log_output(self._process.stderr, self._logger, self._log_level, self._observer)
